...
template <typename T> class DynamicArray // zdefinowanie klasy jako szablon 
{
    ...
    public:
        ...
        T get(int i){ // Funkcja wyszukująca losową liczbę w liście

            return array[i];
        }

        T front(){ // Funkcja wyszukująca losową liczbę w liście
        ...
        T back(){ // Funkcja wyszukująca losową liczbę w liście

            return array[size - 1];
        }

        void Swap(int i, int j){ // Funkcja wyszukująca losową liczbę w liście
